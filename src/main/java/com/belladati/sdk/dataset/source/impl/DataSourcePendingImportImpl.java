...
public class DataSourcePendingImportImpl extends DataSourceImportBaseImpl implements DataSourcePendingImport {
	...
	public DataSourcePendingImportImpl(BellaDatiServiceImpl service, String sourceId, Date date) {
		super("", date);
	...
	public void post() throws NotFoundException, IllegalStateException {
		...
		service.getClient().post("api/dataSets/dataSources/" + sourceId + "/schedule", service.getTokenHolder(),
			Collections.singletonList(new BasicNameValuePair("params", toJson().toString())));
		posted = true;
	}

	public ImportPeriod findPeriod(ImportInterval interval) {
